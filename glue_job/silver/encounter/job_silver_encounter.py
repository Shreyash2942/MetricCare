...
from pyspark.sql.functions import (
    current_timestamp, col, to_date, split, lit,
    regexp_replace, date_format, concat, when,
...
try:
    ...
    # 2️ Read Bronze Encounter table
    bronze_df = glueContext.create_data_frame.from_catalog(
        database=database_name,
        table_name="bronze_encounter"
    ).filter(col("precombine_ts") > lastprocesstime)

    if bronze_df.count() == 0:
        logger.info("No new encounter records to process from Bronze layer.")
    else:
        logger.info(f"Processing {bronze_df.count()} new encounter records from Bronze layer.")

        # 3️ Transform and clean date formats
        df = (
            bronze_df
            .withColumn("admission_date", to_date(regexp_replace(col("admission_time"), r"T.*", ""), "yyyy-MM-dd"))
            .withColumn("discharge_date", to_date(regexp_replace(col("discharge_time"), r"T.*", ""), "yyyy-MM-dd"))
            .withColumn("encounter_duration_days", datediff(col("discharge_date"), col("admission_date")))
            .withColumn("patient_id", split(col("patient_ref"), "/")[1])
            .withColumn("practitioner_id", split(col("practitioner_ref"), "/")[1])
            .dropDuplicates(["encounter_id"])
        )

        # 4️ Select Silver schema
        silver_df = df.select(
            col("encounter_id"),
            col("patient_id"),
            col("practitioner_id"),
            ...
            col("encounter_type"),
            col("department"),
            col("participant_role"),
            col("hospital_name"),
            col("admission_date"),
            col("discharge_date"),
            col("encounter_duration_days"),
            col("sourcename"),
            col("precombine_ts")
        )

        # 5️ Write Silver Encounter Hudi table
        ...
        logger.info(f" Silver Encounter Job {job_name} completed successfully")
