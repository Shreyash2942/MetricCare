...
try:
    ...
    if not new_files:
        logger.info("No new condition files to process. Exiting job.")
    else:
        logger.info(f"Found {len(new_files)} new condition files")
        ...
        # Flatten FHIR Condition schema
        flat_df = df.select(
            col("id").alias("condition_id"),
            col("subject")["reference"].alias("patient_ref"),
            col("encounter")["reference"].alias("encounter_ref"),
            col("code")["coding"][0]["code"].alias("snomed_code"),
            col("code")["coding"][0]["display"].alias("condition_name"),
            col("clinicalStatus")["text"].alias("clinical_status"),
            col("onsetDateTime").alias("onset_time"),
            col("abatementDateTime").alias("recovery_time"),
            col("note")[0]["text"].alias("category"),
            col("note")[2]["text"].alias("metric_type"),
            col("note")[3]["text"].alias("severity"),
            col("note")[4]["text"].alias("version"),
            col("sourcename"),
            col("precombine_ts")
        ).dropDuplicates(["condition_id"])

        # Write to Hudi Bronze table
        condition_hudi_conf = hudi_options(
            table_name=table_name,
            database_name=database_name,
            recordkey="condition_id",
            precombine="precombine_ts",
            partition_key="clinical_status",
            output_path=output_path
        )
        write_dataframe(flat_df, condition_hudi_conf)
        ...
        timestamp_df = (
            ...
            .withColumn("tabletype", lit("bronzecondition"))
        ...
        logger.info(f"✅ Condition Bronze Job {job_name} completed successfully.")
